...
void BKGEstimationTester()
{
  ...
  for (int j=0; j<THTNumberOfBins; j++)
    {
      ...
      NumberLTHT[j]=(((ecTL-ebTL)/(ecML-ebML))*(NTotalMTHT[j]-ebML*NTotalLTHT[j])-(NTotalTTHT[j]-ebTL*NTotalLTHT[j]))/((elML-ebML)*((ecTL-ebTL)/(ecML-ebML))-(elTL-ebTL));
      NumberCTHT[j]=(((elTL-ebTL)/(elML-ebML))*(NTotalMTHT[j]-ebML*NTotalLTHT[j])-(NTotalTTHT[j]-ebTL*NTotalLTHT[j]))/((ecML-ebML)*((elTL-ebTL)/(elML-ebML))-(ecTL-ebTL));
      NumberBTHT[j]=NTotalLTHT[j]-NumberCTHT[j]-NumberLTHT[j];
  ...
  cout << "Loose    " << NBTHT+NCTHT+NLiTHT << " " << NBTHT << " " << NCTHT << " " << NLiTHT << endl;
  cout << "Medium   " << ebML*NBTHT+ecML*NCTHT+elML*NLiTHT << " " << ebML*NBTHT << " " << ecML*NCTHT << " " << elML*NLiTHT << endl;
  cout << "Tight    " << ebTL*NBTHT+ecTL*NCTHT+elTL*NLiTHT << " " << ebTL*NBTHT << " " << ecTL*NCTHT << " " << elTL*NLiTHT << endl;
  ...
  //Plotting!
  char FileName[100];
  sprintf(FileName,"TestingEstimationProcedure_SUFFIXLOOSE_SUFFIXMEDIUM_SUFFIXTIGHT.pdf");
