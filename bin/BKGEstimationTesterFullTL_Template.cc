...
void BKGEstimationTesterTL()
{
  ...
  for (int j=0; j<THTNumberOfBins; j++) {NTotalLTHT[j]=0; NTotalTTHT[j]=0;}
  
  float FullIntegralT=0; float FullIntegralL=0;
  ...
  float eb=0.9885057471; float el=0.165611072;
  ...
  cout << "Loose    " << NBTHT+NSTHT << " " << NBTHT << " " << NSTHT << endl;
